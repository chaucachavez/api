...
{    
    ...
    public $titulo = 'CONSULTAS POR ASEGURADORA Y PLAN'; 
    public $request;  
    function Footer() 
    {            
        $this->SetY(-10);
...
class consultasseisController extends Controller 
{    
    ...
    {   
        ...
        foreach($datacita as $row){  
            $planes[$row->idaseguradoraplan] = !empty($row->nombreaseguradoraplan) ? $row->nombreaseguradoraplan : "(Vacío)";
            ...
            if(!isset($planesvalor[$row->idaseguradora][$row->idaseguradoraplan][$estadocita]))
                $planesvalor[$row->idaseguradora][$row->idaseguradoraplan][$estadocita] = 0; 
            
            $planesvalor[$row->idaseguradora][$row->idaseguradoraplan][$estadocita] += 1;  
        }  
        
        
        $quiebre = array('idaseguradora' => 'nombreaseguradora');                
        $database = $this->agruparPorColumna($datacita, '', $quiebre, ['idaseguradora'=>'nombreaseguradora']);        
        $datacitaxaseguradora = $this->agruparPorColumna($datacita, '', $quiebre, '', ['idestado', array('6'=>'ATENDIDO','7'=>'CANCELADO', '48'=>'NOASISTIO', '4,5'=>'PORATENDER')]);
         
        $data3 = array();
        foreach($datacitaxaseguradora as $row){  
        ...
        foreach($database as $row){    
            ...
            //Planes
            $j = 0;
            foreach($planes as $idplan => $nombre){ 
                $database[$i]['PLANES'][$j]['nombre']  = $nombre; 
                $database[$i]['PLANES'][$j]['ATENDIDO']  = isset($planesvalor[$row['quiebre']][$idplan]['ATENDIDO']) ? $planesvalor[$row['quiebre']][$idplan]['ATENDIDO'] : 0;
                $database[$i]['PLANES'][$j]['CANCELADO']  = isset($planesvalor[$row['quiebre']][$idplan]['CANCELADO']) ? $planesvalor[$row['quiebre']][$idplan]['CANCELADO'] : 0;
                $database[$i]['PLANES'][$j]['NOASISTIO']  = isset($planesvalor[$row['quiebre']][$idplan]['NOASISTIO']) ? $planesvalor[$row['quiebre']][$idplan]['NOASISTIO'] : 0;
                $database[$i]['PLANES'][$j]['PORATENDER']  = isset($planesvalor[$row['quiebre']][$idplan]['PORATENDER']) ? $planesvalor[$row['quiebre']][$idplan]['PORATENDER'] : 0;

                $total = $database[$i]['PLANES'][$j]['ATENDIDO'] + $database[$i]['PLANES'][$j]['CANCELADO'] + $database[$i]['PLANES'][$j]['NOASISTIO'] + $database[$i]['PLANES'][$j]['PORATENDER'];
                $database[$i]['PLANES'][$j]['total']  = $total;
                $j = $j + 1;
            } 
        ...
        $this->pdf->Cell(50, 10, utf8_decode('Aseguradora / Plan'), 1, 0, 'L', true); 
        ...
        foreach ($database as $row) {
            ...
            $this->pdf->Cell(50, 5, !empty($row['nombreaseguradora'])?utf8_decode($row['nombreaseguradora']):utf8_decode('(Particular)'), 1, 0, 'L', true);  
    ...
    private function devolverTratamientos($datacita) {
        ...
        $coaseguos = \DB::table('cicloautorizacion')
            ->select('cicloautorizacion.idcicloatencion', 'aseguradora.idaseguradora', 'aseguradora.nombre as nombreaseguradora', 'cicloautorizacion.deducible', 
                     'cicloautorizacion.coaseguro', 'aseguradoraplan.idaseguradoraplan', 'aseguradoraplan.nombre as nombreaseguradoraplan')
        ...
        foreach($datacita as $row){ 
            ...
            $row->idaseguradora = $tmpcoa ? $tmpcoa->idaseguradora : null;
            $row->idaseguradoraplan = $tmpcoa ? $tmpcoa->idaseguradoraplan : null;
